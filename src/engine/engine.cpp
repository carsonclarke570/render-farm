...
int engine_init(Game* game) {
    ...
    int32_t err = window_create(game->m_window, WIN_WIDTH, WIN_HEIGHT, WIN_TITLE, WIN_FULL);
    if (err) {
        return err;
    }
...
int engine_run(Game* game) {
    ...
    input_init(game->m_input, game->m_window);
    input_cursor_mode(game->m_window, CURSOR_DISABLED);
    game->init();
    ...
    while (!window_should_close(game->m_window)) {
        ...
        // Process input
        input_poll(game->m_input, game->m_window);

        // Update/Render logic
        game->update(delta);
        while (count >= ENG_FRAME_TIME) {
            glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
            game->render(ENG_FRAME_TIME);

            // Swap buffers
            glfwSwapBuffers(game->m_window->window);
